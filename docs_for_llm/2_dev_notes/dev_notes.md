...
### 📅 2025-12-19 19:39
#### 🧪 任务：Links assembly 参数修复
**[计划阶段]**  
- 修正删除空 section 时的 key 使用，避免未定义变量。  
- 时间戳：2025-12-19 19:39:19 CST。  

**[开发阶段]**  
- `frontend/app/api/_lib/links/assembly.ts`：改为遍历 `sectionMap.entries()`，使用 `sectionSlug` 删除空 section。  

**[问题与解决]**  
- 无。  

**[总结与下步计划]**  
- 重新执行 build/lint 验证通过。  

### 📅 2025-12-19 19:38
#### 🧪 任务：Links assembly lint 继续清理
**[计划阶段]**  
- 移除未使用的回调参数，满足 ESLint no-unused-vars。  
- 时间戳：2025-12-19 19:38:26 CST。  

**[开发阶段]**  
- `frontend/app/api/_lib/links/assembly.ts`：forEach/map 去掉未使用的 `sectionSlug` 参数。  
...
### 📅 2025-12-19 19:36
#### 🧪 任务：Build lint 修复（unused vars）
**[计划阶段]**  
- 清除未使用变量以通过 lint。  
- 时间戳：2025-12-19 19:36:59 CST。  

**[开发阶段]**  
- `frontend/app/api/_lib/links/assembly.ts`：未使用的 `sectionSlug` 参数改为 `_sectionSlug`。  
- `frontend/lib/articles.ts`：移除未使用的 `API_BASE` 常量，并更新注释。  
...
### 📅 2025-12-19 19:30
#### 🧪 任务：BFF 路由迁移至 /api/bff
**[计划阶段]**  
- 将 BFF Route Handlers 迁移到 `/api/bff/*`，避免与 Payload REST `/api/*` 冲突。  
- 时间戳：2025-12-19 19:30:49 CST。  

**[开发阶段]**  
- 路由移动：`frontend/app/api/*` → `frontend/app/api/bff/*`（articles/papers/links/timeline）。  
- 路由导入修正：更新 BFF route handler 内相对路径指向 `api/_lib`。  
- 前端请求改为 `/api/bff/*`：articles list/detail、links、papers、timeline。  
- 测试路径同步：更新 `__tests__/api.*.test.ts` 与 `frontend/e2e/site.spec.ts` 路径与 import。  

**[问题与解决]**  
- 无。  

**[总结与下步计划]**  
- 验证 Admin 写操作回到 `/api/*` 且不再 405；前端页面走 `/api/bff/*` 正常加载。  

### 📅 2025-12-19 19:21
#### 🧪 任务：Phase 6-3 Articles Detail HTML 合同调整
**[计划阶段]**  
- 详情内容统一为 Payload rich text HTML；BFF 返回 `contentHtml`，前端仅渲染 HTML。  
- 时间戳：2025-12-19 19:21:16 CST。  

**[开发阶段]**  
- `frontend/app/api/_lib/articles/responses.ts`：detail 响应改为 `contentHtml`。  
- `frontend/app/api/_lib/articles/types.ts`：detail type 改为 `contentHtml` 字段。  
- `frontend/lib/articles.ts`：detail 类型同步为 `contentHtml`。  
- `frontend/app/articles/[slug]/page.tsx`：移除 ReactMarkdown，新增 HTML 渲染组件（`dangerouslySetInnerHTML`）。  

**[问题与解决]**  
- 无。  

**[总结与下步计划]**  
- 验证详情页 HTML 正文渲染与 404 行为；等待确认后进入下一阶段。  

### 📅 2025-12-19 19:16
#### 🧪 任务：Phase 6-3 Articles Detail 切换至 BFF
**[计划阶段]**  
- 仅切换 detail 数据源到同域 `/api/articles/[slug]`，保持 markdown 渲染与严格 slug 行为。  
- 时间戳：2025-12-19 19:16:23 CST。  

**[开发阶段]**  
- `frontend/lib/articles.ts`：`fetchArticle` 在服务端用 `x-forwarded-proto` + `host` 构造绝对 URL，浏览器端保持相对路径。  

**[问题与解决]**  
- 无。  

**[总结与下步计划]**  
- 复测详情页同域 `/api/articles/[slug]`、markdown 渲染与 404 行为；等待确认后再进入后续阶段。  

### 📅 2025-12-19 19:09
#### 🧪 任务：Phase 6-2 Articles List SSR 绝对 URL 适配
**[计划阶段]**  
- 修复 Node 侧 `fetch` 无法解析相对路径的问题，保持浏览器端使用相对 `/api/articles`。  
- 时间戳：2025-12-19 19:09:08 CST。  

**[开发阶段]**  
- `frontend/lib/articles.ts`：在服务端通过 `x-forwarded-proto` + `host` 构造绝对 URL，请求 `/api/articles`；浏览器端保持相对路径。  

**[问题与解决]**  
- 无。  

**[总结与下步计划]**  
- 复测 `/articles` SSR 请求命中同域 `/api/articles`，浏览器导航仍为相对请求；等待 Phase 6-3。  

### 📅 2025-12-19 19:06
#### 🧪 任务：Phase 6-2 Articles List 切换至 BFF
**[计划阶段]**  
- 仅切换 list 数据源到同域 `/api/articles`，detail 保持 legacy 并明确 Phase 6-3 目标。  
- 时间戳：2025-12-19 19:06:05 CST。  

**[开发阶段]**  
- `frontend/lib/articles.ts`：`fetchArticles` 改用 `/api/articles`；`fetchArticle` 保持 legacy，并添加 Phase 6-3 注释。  

**[问题与解决]**  
- 无。  

**[总结与下步计划]**  
- 复测 `/articles` 列表请求命中同域 `/api/articles`，确认排序与 published 行为一致；等待进入 Phase 6-3。  

### 📅 2025-12-19 19:02
#### 🧪 任务：Papers limit 过渡统一（Phase 6-1）
**[计划阶段]**  
- 暂不引入分页 UI，先统一 SSR/CSR 的 limit 以覆盖当前数据量。  
- 时间戳：2025-12-19 19:02:58 CST。  

**[开发阶段]**  
- `frontend/app/(site)/papers/page.tsx`：标注 Phase 6 过渡策略注释。  
- `frontend/app/(site)/papers/PapersCatalog.tsx`：limit 从 24 提升到 100，并注明 Phase 6 过渡策略。  

**[问题与解决]**  
- 无。  

**[总结与下步计划]**  
- 验证 `/papers` 列表数量与 meta 对齐；后续再引入分页 UI。  

### 📅 2025-12-19 19:02
#### 🧪 任务：Papers tag 解析修正（Phase 6-1）
**[计划阶段]**  
- 修正 tags 参数解析，确保逗号分隔值能被拆分为独立 tag。  
- 时间戳：2025-12-19 19:02:00 CST。  

**[开发阶段]**  
- `frontend/app/api/_lib/papers/query.ts`：`parseTags` 在数组输入时按逗号拆分，避免将 `tags=a,b` 当作单一 token。  

**[问题与解决]**  
- 无。  

**[总结与下步计划]**  
- 复测 `/papers` tag 筛选是否生效；确认后进入 Papers limit 过渡策略。  

### 📅 2025-12-19 19:00
#### 🧪 任务：Papers tag fallback（Phase 6-1）
**[计划阶段]**  
- 当 tag lookup 结果为空时不直接返回空集，改为回退到 papers.tags 关系过滤。  
- 时间戳：2025-12-19 19:00:09 CST。  

**[开发阶段]**  
- `frontend/app/api/papers/route.ts`：lookup 空结果时传 `undefined`，不提前返回空响应。  
- `frontend/app/api/_lib/papers/payload.ts`：移除 tagIds 为空即返回空集的短路逻辑。  

**[问题与解决]**  
- 无。  

**[总结与下步计划]**  
- 验证 `/papers` tag 筛选能返回数据；通过后进入 Papers limit 过渡策略。  

### 📅 2025-12-19 18:58
#### 🧪 任务：Links 过滤下拉数据源修复
**[计划阶段]**  
- 修复 links 分类下拉框未读取到客户端数据的问题，保持单文件最小改动。  
- 时间戳：2025-12-19 18:58:37 CST。  

**[开发阶段]**  
- `frontend/app/(site)/links/LinksDirectory.tsx`：分类下拉框 options 改用 `sectionsData`（客户端 fetch 后的数据），避免仅依赖 SSR 初始 `preparedSections`。  

**[问题与解决]**  
- 无。  

**[总结与下步计划]**  
- 复测 `/links` 分类下拉可见并可筛选；确认后继续 Papers tag fallback。  

### 📅 2025-12-19 18:55
#### 🧪 任务：Phase 6-1 Links BFF fallback 修复
**[计划阶段]**  
- 基于诊断结果（links/group/section/slug 均完整），在 BFF assembly 增加最小 fallback 以兼容关系仅返回 ID 的情况。  
- 时间戳：2025-12-19 18:55:21 CST。  

**[开发阶段]**  
- 更新 `frontend/app/api/_lib/links/assembly.ts`：组装时优先 slug，缺失则回退到 id 作为 key，并保持前端结构不变。  

**[问题与解决]**  
- 未提交 commit（按指示跳过）。  

**[总结与下步计划]**  
- 等待验证 `/links` 页面显示链接卡片；确认后进入 Papers tag fallback。  

### 📅 2025-12-19 18:19
#### 🧪 任务：Phase 6-1 模块级语义对账（Papers/Links）
**[计划阶段]**  
- 对齐前端与 BFF 的字段/分页/筛选语义，定位 Papers 与 Links 的不一致点并给出修复侧建议。  
- 时间戳：2025-12-19 18:19:46 CST。  

**[开发阶段]**  
- 读取 `frontend/app/api/_lib/papers/{query,payload,responses}.ts` 与 `frontend/app/(site)/papers/PapersCatalog.tsx`，核对 pagination 与 tags 参数语义。  
- 读取 `frontend/app/api/_lib/links/{payload,assembly,responses}.ts` 与 `frontend/app/(site)/links/{page,LinksDirectory,types}.tsx`，核对嵌套结构与关系依赖。  

**[问题与解决]**  
- 暂无代码修改；仅输出对账结论与修复侧建议。  

**[总结与下步计划]**  
- 等待确认后再进入 Phase 6-1 的修复阶段（不在本次执行）。  
...
**[计划阶段]**  
- 确认默认无 status 参数时需强制 `_status=published`，避免草稿/未发布泄露。  
- 补充 parity 测试确保返回项 `publishedAt` 非空，验证列表默认行为。  

**[开发阶段]**  
- `frontend/app/api/_lib/articles/payload.ts`：默认/显式 published 时强制 `_status='published'`，draft/review 映射 `_status='draft'`；新增 dev/test guard，检测 published 列表中 `publishedAt` 为空则测试环境抛错、非生产警告。  
- `frontend/__tests__/api.articles.test.ts`：新增“默认 published 且 publishedAt 非空”测试，覆盖调用入参与返回数据校验。  

**[问题与解决]**  
- 无阻塞。  

**[总结与下步计划]**  
- 文章列表默认已锁定 published 且在测试环境阻止缺失 `publishedAt` 的数据；若上线前发现接口仍返回草稿，需检查 Payload 数据 `_status` 字段并重新发布。  
- 后续可在验收脚本中复用此检查，确保回归不再出现。  

### 📅 2025-12-18 21:30
#### 🧪 任务：Links BFF payload depth 修复
**[计划阶段]**  
- 排查 `/api/links` 返回空数据的原因，确认是否因 Payload 查询未填充关系导致装配被裁剪。  
- 计划提高 links/groups 查询 depth 以获取 group/section slug。  

**[开发阶段]**  
- 更新 `frontend/app/api/_lib/links/payload.ts`：links 与 groups 查询改为 `depth: 1`，并添加注释说明非零 depth 以填充关系避免装配被清空。  

**[问题与解决]**  
- 未遇到阻塞；尝试本地 `curl` 时 3000 端口未运行，暂无法直接验证响应。  

**[总结与下步计划]**  
- 待启动 Next 服务器后运行 `curl -x \"\" http://127.0.0.1:3000/api/links` 验证返回数据；如仍为空需检查 Payload 记录中的 group/section 关系与 slug 填充。  
