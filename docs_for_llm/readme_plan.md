...
## 🧩 当前架构
- 前端：`frontend/` 基于 Next.js 14（App Router）+ TypeScript + Tailwind CSS，现代交互页面位于 `app/(dashboard)/`，迁移后的静态站点位于 `app/site/`（/site、/site/history、/site/links、/site/papers），通过 rewrites 将 `/index.html`、`/history.html`、`/links.html`、`/papers.html` 对应至新版路由。
...
- ✅ `/site/links` 与 `/site/papers` 接入后端 API，提供结构化友情链接与论文目录浏览体验
